const { dateFormat, bubbleSort2 } = require('../utils')
const fs = require('fs')
const news = JSON.parse(fs.readFileSync('./record.json', 'utf8'))
class Player {
  constructor (data) {
    this.name = data.name
    ...
    this.health = data.health // 0: 伤停 1: 上场
...
class League {
  ...
  static include (league) {
    const arr = ['JLeague', 'Eliteserien', 'KLeague', 'EuropeLeague']
...
class Match {
  ...
  findGivenSecondPosition (status, position) {
    ...
    // 分拣
    this[status+'TeamInfo'].players.forEach(player => {
      if (player.position.length >=2 && player.position[1].toUpperCase() === position) {
        players.push(player)
      }
    })
    // 排序
  ...
  chooseOne (status, players, index, position, except) {
    ...
    for (let i = 0; i < players[index].length; i++) {
      if (players[index][i].position[0] === position &&
  ...
  chooseOneSub (players, index, position1, position2, except) {
    ...
    if (!position1) {
      ...
      if (position1.charAt(0) === '!') {
        for (let i = 0; i < players[index].length; i++) {
          if (players[index][i].position[0] !== position1.substring(1, position1.length) &&
            players[index][i].health !== 0 &&
            except.indexOf(players[index][i].name) === -1) {
            choosedPlayer = choosedPlayer + players[index][i].name + ', '
            break
        ...
        for (let i = 0; i < players[index].length; i++) {
          if ((players[index][i].position[0] === position1 || players[index][i].position[0] === position2) &&
            players[index][i].health !== 0 &&
            except.indexOf(players[index][i].name) === -1) {
            choosedPlayer = choosedPlayer + players[index][i].name + ', '
            break
        ...
        for (let i = 0; i < players[index].length; i++) {
          if (players[index][i].position[0] === position1 &&
            players[index][i].health !== 0 &&
            except.indexOf(players[index][i].name) === -1) {
            choosedPlayer = choosedPlayer + players[index][i].name + ', '
            break
  ...
  getTeamPlayersInfo (status) {
    ...
    for (let i = 0; i < players.length; i++) {
      ...
      for (let j = 0; j < players[i].length; j++) {
        str = str + `${players[i][j].number || '?'}号 ${players[i][j].name}（${players[i][j].position[0]}, ${players[i][j].stats}, ${players[i][j].caps}场${players[i][j].lineups}首发, ${players[i][j].goal ?players[i][j].goal + '球' : ''}${players[i][j].assist ?players[i][j].assist+'助':''}）${players[i][j].age ?'年龄: '+players[i][j].age:''} ${players[i][j].height ?'身高: '+players[i][j].height:''} ${players[i][j].description || ''}` + '\n'
      }
    }
    return str
  }
  ...
  getTeamFutureMatchInfo (status) {
    ...
    this[status + 'TeamFutureMatchs'].forEach(match => {
      ...
      str = str + `${match.league} ${dateFormat(match.matchTime, 'MM-DD HH:mm')} ${match.match} ${match.period}时\n`
    })
    return str
  }
